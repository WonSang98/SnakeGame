...
public:
  ...
  //Check Function
  bool CrushBody(); // Snake의 head가 Body에 닿았는지 체크.
...
//Check Function
bool Snake::CrushBody(){ // Snake의 head가 Body에 닿았는지 체크.
  for(int i=1; i<body.size(); i++){
...
bool Snake::UnableItem(int p1, int p2){ // 아이템이 생성 가능한 지 확인.
  for(int i=0; i<body.size(); i++){
    if(body[i].first == p1 && body[i].second == p2) return false;
  }
  return true;
}
